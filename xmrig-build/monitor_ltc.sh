...
echo "Threads:"
THREAD=$(grep "READY threads" miner_ltc.log | tail -1)
if [ -z "$THREAD" ]; then
    echo "Waiting..."
else
    echo "$THREAD"
...
echo "Hashrate:"
HASH=$(grep "miner    speed" miner_ltc.log | tail -1)
...
ACCEPT=$(grep -c "accepted" miner_ltc.log)
REJECT=$(grep -c "rejected" miner_ltc.log)
